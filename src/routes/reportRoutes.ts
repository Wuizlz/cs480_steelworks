...
import { Pool } from "pg";
import { getFlagCounts, getWeeklyDetails, getWeeklySummary } from "../services/reportService";
...
export function createReportRouter(pool: Pool): Router {
  ...
  router.get("/weekly-summary", asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!parseIsoDateUTC(rangeStart) || !parseIsoDateUTC(rangeEnd)) {
      res.status(400).json({ error: "start_week and end_week must be YYYY-MM-DD" });
      return;
    }
  ...
  router.get("/weekly-details", asyncHandler(async (req: Request, res: Response) => {
    ...
    const defectType = typeof req.query.defect_type === "string" ? req.query.defect_type : null;

    // Validate required params.
    if (!weekStart || !lineName || !defectType) {
      res.status(400).json({ error: "week_start, line_name, and defect_type are required" });
      return;
    }
    ...
    const details = await getWeeklyDetails(pool, weekStart, lineName, defectType);
