 ...
 * Space complexity: O(1).
 */
async function normalizeLabel(client: PoolClient, label: string): Promise<string | null> {
 ...
 * Space complexity: O(1).
 */
async function normalizeLotId(client: PoolClient, lotIdRaw: string): Promise<string | null> {
...
async function ensureIssueType(
  ...
  const result = await client.query(UPSERT_ISSUE_TYPE_SQL, [source, issueLabel, normalized]);
...
async function resolveLotKey(
  ...
  if (params.source === "PRODUCTION_LOG") {
    await client.query(UPDATE_PROD_LOG_LOT_SQL, [resolvedKey, params.sourceKey]);
  } else {
    await client.query(UPDATE_SHIP_LOG_LOT_SQL, [resolvedKey, params.sourceKey]);
  }
...
export async function processProductionLogs(
  ...
  try {
    ...
    const rowsResult = await client.query(UNPROCESSED_PRODUCTION_LOGS_SQL, [batchSize]);
    ...
    for (const row of rowsResult.rows) {
      ...
      const assignmentResult = await client.query(FIND_LOT_ASSIGNMENT_SQL, [lotResolution.lotKey]);
      const assignedLineKey = assignmentResult.rows[0]?.production_line_key ?? null;

      if (assignedLineKey && Number(assignedLineKey) !== Number(row.production_line_key)) {
        await insertFlag(client, {
          flagType: "CONFLICT",
          source: "PRODUCTION_LOG",
      ...
      if (!assignedLineKey) {
        ...
        await client.query(INSERT_LOT_ASSIGNMENT_SQL, [lotResolution.lotKey, row.production_line_key]);
      }
...
export async function processShippingLogs(
  ...
  try {
    ...
    const rowsResult = await client.query(UNPROCESSED_SHIPPING_LOGS_SQL, [batchSize]);
 ...
 */
export async function processAllLogs(pool: Pool, batchSize = DEFAULT_BATCH_SIZE): Promise<{ production: ProcessResult; shipping: ProcessResult }> {
