...
async function resolveLotKey(client, params) {
  ...
  if (params.lotKey) {
    const lotRow = await client.query(queries_1.FIND_LOT_BY_KEY_SQL, [
      params.lotKey,
    ]);
...
async function processProductionLogs(pool, batchSize = DEFAULT_BATCH_SIZE) {
  ...
  try {
    ...
    const rowsResult = await client.query(
      queries_1.UNPROCESSED_PRODUCTION_LOGS_SQL,
      [batchSize],
    );
    ...
    for (const row of rowsResult.rows) {
      ...
      const assignmentResult = await client.query(
        queries_1.FIND_LOT_ASSIGNMENT_SQL,
      ...
      const weekStart = (0, date_1.toIsoDateUTC)(
        (0, date_1.getWeekStartUTC)(runDate),
      );
...
async function processShippingLogs(pool, batchSize = DEFAULT_BATCH_SIZE) {
  ...
  try {
    ...
    const rowsResult = await client.query(
      queries_1.UNPROCESSED_SHIPPING_LOGS_SQL,
      [batchSize],
    );
    fetched = rowsResult.rows.length;
    for (const row of rowsResult.rows) {
      ...
      const weekStart = (0, date_1.toIsoDateUTC)(
        (0, date_1.getWeekStartUTC)(shipDate),
      );
      // Insert the issue event for reporting.
      await client.query(queries_1.INSERT_ISSUE_EVENT_SQL, [
        "SHIPPING",
