...
exports.processProductionLogs = processProductionLogs;
exports.processShippingLogs = processShippingLogs;
exports.processAllLogs = processAllLogs;
const queries_1 = require("../db/queries");
const logger_1 = require("../logging/logger");
const date_1 = require("../utils/date");
// Batch size default keeps per-transaction work bounded.
const DEFAULT_BATCH_SIZE = 500;
const logger = (0, logger_1.getLogger)("service.ingest");
/**
...
function coerceDateToIso(value) {
    ...
    if (value instanceof Date) {
        return (0, date_1.toIsoDateUTC)(value);
    }
 ...
 */
async function insertFlag(client, params) {
    await client.query(queries_1.INSERT_DQ_FLAG_SQL, [
        params.flagType,
        params.source,
        params.reason,
 ...
 * Space complexity: O(1).
 */
async function normalizeLabel(client, label) {
    const result = await client.query(queries_1.NORMALIZE_LABEL_SQL, [label]);
 ...
 * Space complexity: O(1).
 */
async function normalizeLotId(client, lotIdRaw) {
    const result = await client.query(queries_1.NORMALIZE_LOT_ID_SQL, [lotIdRaw]);
 ...
 * Space complexity: O(1).
 */
async function ensureIssueType(client, source, issueLabel) {
    ...
    const result = await client.query(queries_1.UPSERT_ISSUE_TYPE_SQL, [
        source,
        issueLabel,
        normalized,
    ]);
 ...
 * Space complexity: O(1).
 */
async function resolveLotKey(client, params) {
    // If the lot_key already exists, reuse it for traceability.
    if (params.lotKey) {
        const lotRow = await client.query(queries_1.FIND_LOT_BY_KEY_SQL, [params.lotKey]);
    ...
    const lotResult = await client.query(queries_1.FIND_LOT_SQL, [lotIdNorm]);
    ...
    if (params.source === "PRODUCTION_LOG") {
        await client.query(queries_1.UPDATE_PROD_LOG_LOT_SQL, [
            resolvedKey,
            params.sourceKey,
        ]);
    }
    else {
        await client.query(queries_1.UPDATE_SHIP_LOG_LOT_SQL, [
            resolvedKey,
            params.sourceKey,
        ]);
    }
 ...
 * Space complexity: O(1).
 */
function deriveProductionQty(row) {
 ...
 */
async function processProductionLogs(pool, batchSize = DEFAULT_BATCH_SIZE) {
    let processed = 0;
    let flagged = 0;
    let fetched = 0;
    ...
    try {
        ...
        const rowsResult = await client.query(queries_1.UNPROCESSED_PRODUCTION_LOGS_SQL, [
            batchSize,
        ]);
        ...
        for (const row of rowsResult.rows) {
            ...
            const issueTypeKey = await ensureIssueType(client, "PRODUCTION", row.primary_issue);
            if (issueTypeKey < 0) {
                await insertFlag(client, {
                    flagType: "INCOMPLETE_DATA",
                    source: "PRODUCTION_LOG",
            ...
            const assignmentResult = await client.query(queries_1.FIND_LOT_ASSIGNMENT_SQL, [
                lotResolution.lotKey,
            ]);
            const assignedLineKey = assignmentResult.rows[0]?.production_line_key ?? null;
            if (assignedLineKey &&
            ...
            if (!assignedLineKey) {
                ...
                await client.query(queries_1.INSERT_LOT_ASSIGNMENT_SQL, [
                    lotResolution.lotKey,
            ...
            const runDate = (0, date_1.parseIsoDateUTC)(runDateIso);
            if (!runDate) {
                await insertFlag(client, {
                    flagType: "INCOMPLETE_DATA",
                    source: "PRODUCTION_LOG",
            ...
            const weekStart = (0, date_1.toIsoDateUTC)((0, date_1.getWeekStartUTC)(runDate));
            // Insert the issue event for reporting.
            await client.query(queries_1.INSERT_ISSUE_EVENT_SQL, [
                "PRODUCTION",
                runDateIso,
                weekStart,
                row.production_line_key,
 ...
 * Space complexity: O(1) aside from the input batch.
 */
async function processShippingLogs(pool, batchSize = DEFAULT_BATCH_SIZE) {
    let processed = 0;
    let flagged = 0;
    let fetched = 0;
    ...
    try {
        ...
        const rowsResult = await client.query(queries_1.UNPROCESSED_SHIPPING_LOGS_SQL, [
            batchSize,
        ]);
        fetched = rowsResult.rows.length;
        for (const row of rowsResult.rows) {
            ...
            // Ensure the issue type exists and fetch its key.
            const issueTypeKey = await ensureIssueType(client, "SHIPPING", row.hold_reason);
            if (issueTypeKey < 0) {
                await insertFlag(client, {
                    flagType: "INCOMPLETE_DATA",
                    source: "SHIPPING_LOG",
            ...
            const shipDate = (0, date_1.parseIsoDateUTC)(shipDateIso);
            if (!shipDate) {
                await insertFlag(client, {
                    flagType: "INCOMPLETE_DATA",
                    source: "SHIPPING_LOG",
            ...
            const weekStart = (0, date_1.toIsoDateUTC)((0, date_1.getWeekStartUTC)(shipDate));
            // Insert the issue event for reporting.
            await client.query(queries_1.INSERT_ISSUE_EVENT_SQL, [
                "SHIPPING",
                shipDateIso,
 ...
 */
async function processAllLogs(pool, batchSize = DEFAULT_BATCH_SIZE) {
