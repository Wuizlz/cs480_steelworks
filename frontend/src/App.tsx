...
export default function App(): JSX.Element {
  ...
  const loadSummary = useCallback(
    async (range?: { start_week?: string; end_week?: string }) => {
      ...
      try {
        ...
        const data = await fetchJson<WeeklySummaryResponse>(
          `/reports/weekly-summary${query}`,
        );
  ...
  const loadDetails = useCallback(
    async (filters: {
      ...
      try {
        ...
        const data = await fetchJson<WeeklyDetailResponse>(
          `/reports/weekly-details${query}`,
        );
  ...
  const loadFlags = useCallback(
    async (range?: { start_week?: string; end_week?: string }) => {
      ...
      try {
        ...
        const data = await fetchJson<FlagCountResponse>(
          `/reports/flags${query}`,
        );
  ...
  const runJob = useCallback(async (batchSize?: number) => {
    ...
    try {
      ...
      const processedTotal =
        data.production.processed + data.shipping.processed;
    ...
    <div className="app">
      ...
      <section className="panel">
        <div className="panel-header">
          ...
          <p className="panel-help">
            Select a week range, then click a row to drill down.
          </p>
        </div>
        <form
          className="form-grid"
      ...
      <section className="panel">
        <div className="panel-header">
          ...
          <p className="panel-help">
            Use the summary table or enter values below.
          </p>
        </div>
        <form
          className="form-grid"
        ...
        >
          <label>
            ...
            <input
              type="date"
              value={detailWeek}
              onChange={(event) => setDetailWeek(event.target.value)}
              required
            />
          </label>
          <label>
          ...
          <table>
            ...
            <tbody>
              {detailRows.length === 0 ? (
                ...
                detailRows.map((row) => (
                  <tr
                    key={`${row.event_source}-${row.event_date}-${row.lot_id_norm}`}
                  >
      ...
      <section className="panel">
        <div className="panel-header">
          ...
          <p className="panel-help">
            Monitor excluded records by week and reason.
          </p>
        </div>
        <form
          className="form-grid"
        ...
        >
          <label>
            ...
            <input
              type="date"
              value={flagsStart}
              onChange={(event) => setFlagsStart(event.target.value)}
            />
          </label>
          <label>
            <span>End week</span>
            <input
              type="date"
              value={flagsEnd}
              onChange={(event) => setFlagsEnd(event.target.value)}
            />
          </label>
      ...
      <section className="panel">
        <div className="panel-header">
          <h2>Process Logs</h2>
          <p className="panel-help">
            Trigger data-quality processing to refresh the report.
          </p>
        </div>
        <form
          className="form-grid"
          onSubmit={(event) => {
            ...
            const batchSize =
              Number.isFinite(batchValue) && batchValue > 0
                ? batchValue
                : undefined;
